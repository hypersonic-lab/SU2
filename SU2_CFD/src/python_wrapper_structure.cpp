...
////////////////////////////////////////////////////////////////////////////////
/* Functions that were present in su2-adapter for old version preCICE */
////////////////////////////////////////////////////////////////////////////////
...
void CDriver::SetUnsteady_TimeStep(passivedouble val_delta_unsttime) {
    const passivedouble time_ref = config_container[ZONE_0]->GetTime_Ref();

    const passivedouble delta_t_nd = val_delta_unsttime / time_ref;

  std::cout << std::setprecision(16) << "[SetUnsteady_TimeStep] " << "dt_dim = " << val_delta_unsttime << ", time_ref = " << time_ref << ", dt_nd = " << delta_t_nd << std::endl;
...
void CDriver::ReloadOldState() {
  ...
  const bool heat_solver = config_container[ZONE_0]->GetKind_Solver() == MAIN_SOLVER::HEAT_EQUATION;
  const unsigned short primary_solver = heat_solver ? HEAT_SOL : FLOW_SOL;
  auto* primary_solver_ptr = solver_container[ZONE_0][INST_0][MESH_0][primary_solver];
  if (primary_solver_ptr == nullptr) {
    SU2_MPI::Error("Primary solver pointer is null in preCICE checkpoint routine.", CURRENT_FUNCTION);
  }

  // Get the number of solution variables, points, and dimension
  const unsigned short nVar = primary_solver_ptr->GetnVar();
  ...
  // Get if RANS
  const bool rans = !heat_solver && config_container[ZONE_0]->GetKind_Turb_Model() != TURB_MODEL::NONE;
  ...
  for (unsigned long iPoint_Local = 0; iPoint_Local < nPoint_Local; iPoint_Local++) {
    ...
    for (unsigned short iVar = 0; iVar < nVar; iVar++) {
        primary_solver_ptr->GetNodes()->SetSolution(iPoint_Local, iVar, preCICE_Solution(iPoint_Local, iVar));
        primary_solver_ptr->GetNodes()->Set_Solution_time_n(iPoint_Local, iVar, preCICE_Solution_time_n(iPoint_Local, iVar));
        primary_solver_ptr->GetNodes()->Set_Solution_time_n1(iPoint_Local, iVar, preCICE_Solution_time_n1(iPoint_Local, iVar));
    }
    if (rans) {
  ...
  if (heat_solver) FinalizeHEAT_SOL();
  else FinalizeFLOW_SOL();

  if (rans) FinalizeTURB_SOL();
  if (dynamic_grid) FinalizeMESH_SOL();
}

//preCICE: Finalize HEAT reloads
void CDriver::FinalizeHEAT_SOL() {

  auto* geometry = geometry_container[ZONE_0][INST_0][MESH_0];
  auto* config = config_container[ZONE_0];
  auto* heat_solver = solver_container[ZONE_0][INST_0][MESH_0][HEAT_SOL];

  if (heat_solver == nullptr) {
    SU2_MPI::Error("HEAT_SOL pointer is null while finalizing preCICE checkpoint reload.",CURRENT_FUNCTION);
  }

  heat_solver->InitiateComms(geometry, config, MPI_QUANTITIES::SOLUTION);
  heat_solver->CompleteComms(geometry, config, MPI_QUANTITIES::SOLUTION);

  heat_solver->InitiateComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N);
  heat_solver->CompleteComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N);

  heat_solver->InitiateComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N1);
  heat_solver->CompleteComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N1);
}
...
void CDriver::FinalizeFLOW_SOL() {
  ...
  if (dynamic_grid) {
    ...
    for (auto iMesh = 0u; iMesh <= config_container[ZONE_0]->GetnMGLevels(); iMesh++) {
      ...
      if (iMesh) geometry_container[ZONE_0][INST_0][iMesh]->SetRestricted_GridVelocity(geometry_container[ZONE_0][INST_0][iMesh - 1]);
        ...
        geometry_container[ZONE_0][INST_0][MESH_0]->InitiateComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::GRID_VELOCITY);
        geometry_container[ZONE_0][INST_0][MESH_0]->CompleteComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::GRID_VELOCITY);
      }
    }
  }
  ...
  solver_container[ZONE_0][INST_0][MESH_0][FLOW_SOL]->InitiateComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
  solver_container[ZONE_0][INST_0][MESH_0][FLOW_SOL]->CompleteComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
    ...
    flow_solver->InitiateComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N);
    flow_solver->CompleteComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N);

    flow_solver->InitiateComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N1);
    flow_solver->CompleteComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N1);
  ...
  for (auto iMesh = 1u; iMesh <= config_container[ZONE_0]->GetnMGLevels(); iMesh++) {
    ...
    solver_container[ZONE_0][INST_0][iMesh][FLOW_SOL]->InitiateComms(geometry_container[ZONE_0][INST_0][iMesh], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
    solver_container[ZONE_0][INST_0][iMesh][FLOW_SOL]->CompleteComms(geometry_container[ZONE_0][INST_0][iMesh], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
...
void CDriver::FinalizeTURB_SOL() {
  ...
  solver_container[ZONE_0][INST_0][MESH_0][TURB_SOL]->InitiateComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
  solver_container[ZONE_0][INST_0][MESH_0][TURB_SOL]->CompleteComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
    auto* turb_solver =
        solver_container[ZONE_0][INST_0][MESH_0][TURB_SOL];
    ...
    turb_solver->InitiateComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N);
    turb_solver->CompleteComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N);

    turb_solver->InitiateComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N1);
    turb_solver->CompleteComms(geometry, config, MPI_QUANTITIES::SOLUTION_TIME_N1);
  ...
  for (auto iMesh = 1u; iMesh <= config_container[ZONE_0]->GetnMGLevels(); iMesh++) {
    ...
    solver_container[ZONE_0][INST_0][iMesh][TURB_SOL]->InitiateComms(geometry_container[ZONE_0][INST_0][iMesh], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
    solver_container[ZONE_0][INST_0][iMesh][TURB_SOL]->CompleteComms(geometry_container[ZONE_0][INST_0][iMesh], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
...
void CDriver::FinalizeMESH_SOL() {
  ...
  solver_container[ZONE_0][INST_0][MESH_0][MESH_SOL]->InitiateComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
  solver_container[ZONE_0][INST_0][MESH_0][MESH_SOL]->CompleteComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION);
  ...
  solver_container[ZONE_0][INST_0][MESH_0][MESH_SOL]->InitiateComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION_TIME_N);
  solver_container[ZONE_0][INST_0][MESH_0][MESH_SOL]->CompleteComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION_TIME_N);
  
  solver_container[ZONE_0][INST_0][MESH_0][MESH_SOL]->InitiateComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION_TIME_N1);
  solver_container[ZONE_0][INST_0][MESH_0][MESH_SOL]->CompleteComms(geometry_container[ZONE_0][INST_0][MESH_0], config_container[ZONE_0], MPI_QUANTITIES::SOLUTION_TIME_N1);
...
void CDriver::SaveOldState() {
  ...
  for (unsigned long iPoint_Local = 0; iPoint_Local < nPoint_Local; iPoint_Local++) {
    ...
    for (unsigned short iVar = 0; iVar < nVar; iVar++) {
      preCICE_Solution(iPoint_Local, iVar) = primary_solver_ptr->GetNodes()->GetSolution(iPoint_Local, iVar);
      preCICE_Solution_time_n(iPoint_Local, iVar) = primary_solver_ptr->GetNodes()->GetSolution_time_n(iPoint_Local, iVar);
      preCICE_Solution_time_n1(iPoint_Local, iVar) = primary_solver_ptr->GetNodes()->GetSolution_time_n1(iPoint_Local, iVar);
    }
...
passivedouble CDriver::GetVertexTemperature(unsigned short iMarker,
  ...
  const bool heat_solver =
      config->GetKind_Solver() == MAIN_SOLVER::HEAT_EQUATION;

  CSolver* temperature_solver =
      solver_container[ZONE_0][INST_0][MESH_0]
                      [heat_solver ? HEAT_SOL : FLOW_SOL];

  if (temperature_solver == nullptr) {
    SU2_MPI::Error(
        "Temperature solver pointer is null in GetVertexTemperature().",
        CURRENT_FUNCTION);
  }

  const su2double vertex_temperature =
      temperature_solver->GetNodes()->GetTemperature(iPoint);

  // Re-dimensionalize before sending to preCICE.
  return SU2_TYPE::GetValue(
      vertex_temperature * config->GetTemperature_Ref());
}
...
void CDriver::SetVertexNormalHeatFlux(unsigned short iMarker, unsigned long iVertex, passivedouble val_WallHeatFlux){
  ...
  const passivedouble heat_flux_ref = config_container[ZONE_0]->GetHeat_Flux_Ref();

  const passivedouble heat_flux_nd = val_WallHeatFlux / heat_flux_ref;

  std::cout << std::setprecision(16) << "[SetVertexNormalHeatFlux] " << "q_dim = " << val_WallHeatFlux << ", q_ref = " << heat_flux_ref << ", q_nd = " << heat_flux_nd << std::endl;
...
void CDriver::ResetConvergence() {
  ...
  for(iZone = 0; iZone < nZone; iZone++) {
      ...
      output_container[iZone]->SetConvergence(false);
...
void CDriver::CommunicateMeshDisplacement(void) {
  ...
  solver_container[ZONE_0][INST_0][MESH_0][MESH_SOL]->InitiateComms(geometry_container[ZONE_0][INST_0][MESH_0],
                                                                    config_container[ZONE_0], MPI_QUANTITIES::MESH_DISPLACEMENTS);
