  ...
  for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++)
    switch (config->GetMarker_All_KindBC(iMarker)) {
      case RADIATIVE_EQUILIBRIUM:
        solver_container[MainSolver]->BC_RadiativeEquilibrium_Wall(geometry, solver_container, conv_bound_numerics, visc_bound_numerics, config, iMarker);
        break;
      case ISOTHERMAL:
      ...
      case SMOLUCHOWSKI_MAXWELL:
        ...
        break;
      case ETC:
        solver_container[MainSolver]->BC_ETC_Wall(geometry, solver_container, conv_bound_numerics, visc_bound_numerics, config, iMarker);
        break;
    }
