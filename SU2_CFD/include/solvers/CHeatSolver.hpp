...
protected:
  ...
  vector<vector<su2double>> HeatFluxConvective;
  vector<vector<su2double>> HeatFluxRadiative;
...
public:
   ...
   * \return Value of the heat flux.
   */
  inline su2double GetHeatFluxRadiative(unsigned short val_marker, unsigned long val_vertex) const override {
    return HeatFluxRadiative[val_marker][val_vertex];
  }

  /*!
   * \brief Get the convective heat flux.
   * \param[in] val_marker - Surface marker where the coefficient is computed.
   * \param[in] val_vertex - Vertex of the marker <i>val_marker</i> where the coefficient is evaluated.
   * \return Value of the heat flux.
   */
  inline su2double GetHeatFluxConvective(unsigned short val_marker, unsigned long val_vertex) const override {
